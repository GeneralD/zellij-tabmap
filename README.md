...
default_tab_template {
    pane size=3 borderless=true {                       // 3 rows (the floor); raise to 4+ to enable perspective
        plugin location="file:/Users/you/.config/zellij/plugins/zellij-tabmap.wasm" {
            ...
            scroll_throttle "3"                         // wheel events per step; higher = less sensitive, "1" = one step per event
...
>
> **`scroll` — mouse wheel navigation.** Selects what the wheel does over the bar (zellij scroll events carry no position, so the gesture is bar-wide, not tied to a specific tab). `tab` (default) switches tabs — scroll up = next, scroll down = previous, following zellij's stock tab-bar direction but **wrapping** at the ends (first ↔ last) instead of clamping. `pane` instead walks the **focused pane** forward / backward in reading order (top→bottom, then left→right), crossing tab boundaries — stepping past a tab's last pane jumps to the next tab's first pane, and back — wrapping globally; the focus is absolute, so the bar's highlight follows correctly. `off` leaves the wheel inert. The wheel is throttled by `scroll_throttle` (below) so a stepless device does not race through tabs; the reported line count is otherwise ignored (each event is one notch). No extra permission is needed beyond the default set, so existing installs gain this on update without a re-grant.
>
> **`scroll_throttle` — wheel sensitivity.** How many wheel events one navigation step takes (default `3`). A stepless pointing device — an Apple Magic Mouse, a trackpad — reports a single flick as a *stream* of scroll events, so the original one-event-per-step behavior raced through several tabs (or panes) at once. This adds a software detent: the bar counts events and steps only on every `scroll_throttle`-th one, so one flick advances roughly one tab. Raise it to damp the wheel further; set `1` to restore the original one-step-per-event feel (best for a notched wheel mouse, which already detents in hardware). A reversal of direction clears any leftover count from the opposite way, so the new direction starts a fresh detent instead of first paying down stale momentum (it still takes `scroll_throttle` events the new way to step). Ignored when `scroll "off"`; a value of `0` is treated as `1`. No extra permission needed.
>
