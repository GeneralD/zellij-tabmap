...
echo "Building render_active_cue..."
CARGO_BUILD_TARGET=aarch64-apple-darwin cargo build --example render_active_cue \
...
Enter
Sleep 100ms
Type "$REPO_ROOT/target/aarch64-apple-darwin/debug/examples/render_active_cue"
Enter
Sleep 2s
Show
Sleep 500ms
...
# Extract current constants (empty string if not found)
