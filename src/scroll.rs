...
//!
//! **One wheel event maps to exactly one navigation step.** The rate-limiter
//! that #83 (leading-edge cooldown), #96 (debounce), and #100 (reopen-on-timer)
//! each tried — and that #104 deleted along with the whole feature — is gone for
//! good (#108): no `Gate`, no cooldown window, no `Timer`. zellij emits one
//! scroll event per sub-notch with no device identity, so a stepless device
//! (Magic Mouse, trackpad) fires a *burst* per flick and steps several tabs /
//! panes at once. That is the accepted trade-off of the rate-limiter-free form:
//! users who dislike it opt out with `scroll = off` rather than living with a
//! limiter that never reconciled both device classes.
...
pub enum ScrollMode {
    ...
    Pane,
    /// Scroll does nothing — the opt-out for stepless devices whose flick bursts
    /// step several at once (#108).
    Off,
}
