...
### スパイク結果（実施済み 2026-07-12、zellij 0.44.3）

隔離セッション `tabmap-verify-110` ＋ eprintln オラクル ＋ expect PTY で実機裏取り済み。生ログの要点:

- **P0-1 = YES（最重要・確定）** — `toggle-floating-panes` で層を hide した後も、フロートは
  `PaneManifest` に **id 付き・`is_floating=true` のまま残る**（2フロートで `id=1`/`id=2` が持続）。
  `TabInfo.are_floating_panes_visible` は `false` に倒れるが `selectable_floating_panes_count` は
  `2` を維持。無関係な PaneUpdate（タイル分割）を挟んでも残存。→ **個別チップ＋per-float reveal は
  実装可能。P2 は計画どおり、issue #2 達成可能。**
- **P0-2 = YES（確定）** — 表示中フロートの座標はタイルと同一系（例: フロート `x=30 y=12 w=60 h=18`、
  タイル `x=0 y=3 w=120 h=37`。`y` は `size=3` バー直下から）。→ **オーバーレイ写像 OK。P3 計画どおり。**
- **P0-3 = 不確定** — フォーカスロスでの自動 hide は、検証中に expect PTY が hold 期限切れで detach し
  （"No connected clients to change focus for"）、焦点移動アクションが実行できず未確認。ただし **P0-3 は
  P4（ホイール walk）のみに影響**し、後述の**保守的 P4 設計を採れば auto-hide 挙動に依存せず堅牢**なので
  ブロッカーではない。P4 実装時に必要なら fresh PTY で再測する。

**確定した設計方針:** P2（チップ）・P3（オーバーレイ）は計画のまま。P4 は保守版（walk 対象＝タイル＋
**表示中**フロートのみ、非表示フロートはチップクリックでのみ reveal）を採用 → Task 12 参照。
...
> **P0-1 分岐 — 確定 = YES（スパイクで裏取り済み）。** 非表示フロートは manifest に id 付きで残ることを実機確認したので、以下の個別チップ設計をそのまま実装する。（参考: NO だった場合の縮退案は `FloatLayer::Hidden(usize)`（count）＋カウントバッジ `◰N` だったが、不要になった。）
...
> **P0-3 分岐 — 確定 = 保守版を採用。** 自動 hide は実機で未確認（P0 スパイク参照）。auto-hide 挙動に
> 依存しないよう、walk 対象は **タイル＋その時点で表示中のフロート**のみとする（`are_floating_panes_visible
> == true` のタブのフロートだけを連結）。非表示フロートはホイールでは開かず、チップクリック（P2）だけが
> reveal の入口。これによりホイールで層を不意に開いてしまう挙動が起きず、P0-3 の結果に関わらず正しい。

### Task 12: `pane_focus_order` に**表示中**フロートを合流する
...
`scroll_state` fixture が作る状態にフロートを足す（タブ0の層は可視、タブ1の層は非表示）:
...
fn pane_focus_order_appends_visible_floats_but_not_hidden_ones() {
    ...
    // floats: [10, 20, 15, 30]. Float 25 is excluded — a hidden float is reached
    // via its chip (P2), never the wheel, so wheeling never pops the layer open
    // (#110; P0-3 unconfirmed → auto-hide-agnostic).
    ...
    state.panes.panes.get_mut(&1).unwrap().push(PaneInfo {
        id: 25,
        is_floating: true,
        pane_x: 5,
        pane_y: 5,
...
> **Note:** `scroll_state` の実体（`state.tabs` / `state.panes.panes` の中身）を実装時に確認し、
> タブ0=タイル `[10, 20]`・タブ1=タイル `[30]` になるようフィクスチャを合わせる。異なる場合は
> 期待値をフィクスチャに合わせて調整（walk 順＝position 昇順でタイル→表示中フロート）。
...
- [ ] **Step 3: `pane_focus_order` に**表示中**フロートだけを合流**

`src/lib.rs` の `pane_focus_order`（469–476行目）を、タブごとにタイル→（層が可視なら）フロートの順で連結するよう変更。`focus_terminal_pane` を種別で切り替える必要はない（walk に載るフロートは常に可視なので `should_float_if_hidden` は無関係）— `scroll_panes` は無変更:
    ...
    fn pane_focus_order(&self) -> Vec<u32> {
        ...
        tabs.into_iter()
            .flat_map(|tab| {
                ...
                // Tiled panes in reading order, then — only when this tab's
                // floating layer is currently visible — its floats (#110). Hidden
                // floats are reached via their chip (P2), never the wheel, so the
                // wheel never pops the layer open (auto-hide-agnostic; P0-3).
...
```bash
git add src/lib.rs
git commit -m "feat(lib): walk visible floating panes with the wheel (#110)"
```
