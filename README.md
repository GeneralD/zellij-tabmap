...
>
> **`floating` — show floating panes on the bar.** Controls how a tab's floating panes appear in its minimap (default `hybrid`). Under `hybrid`, the depiction follows the tab's own floating layer: while the layer is **visible**, each float is overlaid graphically on top of the tiled minimap, mapped through the same bounding box so it sits in place without shifting the tiles; while the layer is **hidden**, each float shrinks to a small chip (`◲`) tucked into the block's bottom-right corner (a `⋯` marker stands in for any that overflow the width). **Left-clicking a chip reveals and focuses that hidden float in one step** — the way to reach a hidden floating pane straight from the bar. Clicking the `⋯` overflow marker itself reveals and focuses the first folded float, un-hiding the whole layer so the rest become individually clickable. The mouse wheel's `pane` walk (see `scroll`) steps through a tab's **visible** floats after its tiled panes, but never a hidden one — a hidden float is reached only via its chip, so wheeling never pops a hidden layer open. Set `off` to draw no floating panes at all (the pre-floating look). Uses only the default permission set — no re-grant on update.
