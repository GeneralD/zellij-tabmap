...
>
> **First-run permission note** ([zellij#4982](https://github.com/zellij-org/zellij/issues/4982)): plugins started from `default_tab_template` cannot show the interactive permission dialog, so the bar appears inert on first launch. To grant the permissions, load the plugin **once in a regular pane** — the prompt shows there and the grant is cached per URL:
>
> ```bash
> zellij plugin -- https://github.com/GeneralD/zellij-tabmap/releases/latest/download/zellij-tabmap.wasm
> ```
>
> Press <kbd>y</kbd> to accept, close the pane, and reload — the tab bar comes alive. Note the grant is keyed on the **exact plugin URL**, so a version-pinned URL needs this once per version, while the `latest` URL needs it only once (at the cost of zellij's URL-keyed wasm cache holding back updates). As a fallback you can also add the entry by hand to `permissions.kdl` in zellij's plugin cache directory.
>
