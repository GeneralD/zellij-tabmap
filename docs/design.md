...
load():
  ...
  request_permission([ReadApplicationState])  // v1。v2 で RunActionsAsUser 追加
...
- **権限は最小限**: v1 = `ReadApplicationState`(イベント購読)のみ。クリック切替は Mouse(権限不要)。v2 の D&D で `RunActionsAsUser` を追加。attention の `ReadCliPipes` / `MessageAndLaunchOtherPlugins` はコピーしない。
